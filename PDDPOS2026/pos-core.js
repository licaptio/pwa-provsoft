...
async function guardarVentaEnSegundoPlano(venta) {
  try {
    await addDoc(collection(db, "ventas_rutav2"), venta);
...
function imprimirTicketVenta(v) {
  const anterior = document.getElementById("ticketArea");
  if (anterior) anterior.remove();

  const lineas = v.detalle.map(it => `
    <div style="display:flex;justify-content:space-between;">
      <span>${it.cantidad} ${it.nombre.substring(0,18)}</span>
      <span>${money(it.importe)}</span>
    </div>
  `).join("");

  const html = `
    <div id="ticketArea" style="display:block;background:#fff;">
      <div id="ticket" style="
        width:72mm;
        margin:0 auto;
        padding:4mm 2mm;
      ...
      ">
        ...
        <div style="text-align:center;">PUNTO VENTA 2026</div>
        <hr>
        <div>Folio: ${v.folio}</div>
        <div>Fecha: ${v.fecha_txt}</div>
        <div>Vendedor: ${v.usuarioNombre}</div>
        <hr>
        ${lineas}
        <hr>
        <div>Subtotal: ${money(v.resumen_financiero.subtotal)}</div>
        <div>Impuestos: ${money(v.resumen_financiero.impuestos)}</div>
        <div style="font-weight:bold;font-size:16px;">
          Total: ${money(v.resumen_financiero.total)}
        </div>
        <div>Recibido: ${money(v.recibido)}</div>
        <div>Cambio: ${money(v.cambio)}</div>
        <br>
